...
public:
    ...
    Camera() : position(0.0f, 0.0f, 0.0f), center(0,0,-1),zoom(1.0f) {
        updateViewMatrix();
    }

    void updateViewMatrix() {
        view = glm::lookAt(position, center, glm::vec3(0.0f, 1.0f, 0.0f));
    }
...
int main() {
    ...
    float vertices[] = {
            -0.5f, -0.5f,  // 线段1的起点
            0.5f,  0.5f,  // 线段1的终点
            -0.5f,  0.5f,  // 线段2的终点（与线段1的终点重合）
            0.5f, -0.5f   // 线段2的起点（与线段1的起点重合）
    };
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // 设置投影矩阵
        glm::mat4 projection = glm::ortho(-1.0f * camera.zoom, 1.0f * camera.zoom, -1.0f * camera.zoom * (float)HEIGHT / (float)WIDTH, 1.0f * camera.zoom * (float)HEIGHT / (float)WIDTH, -1.0f, 1.0f);
